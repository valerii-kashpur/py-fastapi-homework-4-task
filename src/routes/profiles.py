...
from database import (
    get_db, UserModel, UserGroupEnum, UserProfileModel, UserGroupModel
)
from exceptions import (
    ...
    S3FileUploadError, S3PermissionError
)
...
async def create_user_profile(
    ...
    if user_id != token_user_id:
        stmt = select(UserModel).where(UserModel.id == token_user_id).options(
    ...
    if profile_data.avatar:
        ...
        except (S3ConnectionError, S3BucketNotFoundError, S3FileUploadError,
                S3PermissionError):
            raise HTTPException(
